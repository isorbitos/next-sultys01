import React from 'react'
// import './pristatymas.css';

const Pristatymas = () => (
    <section className="page-section" id="apieMus">
    <div className="container">
        <div className="text-center">
            <h2 className="section-heading text-uppercase">PRISTATYMAS</h2>
        </div>
        <div className="row text-center">
            <div className="col-md-4">
               
                <h5>Sultis pristatome Vilniaus ribose NEMOKAMAI, perkant už 20 eurų ir daugiau</h5>
            </div>
            <div className="col-md-4">
                
                <h5>Perkant penkias pakuotes, vieną (savo nuožiūra) - dovanojame papildomai</h5>
            </div>
            <div className="col-md-4">
                <h5>Vieną  kartoninę dėžutę dovanojame, jei reikia daugiau - dėžutės kaina 0,50 eur</h5>
            </div>
        </div>
    </div>
</section>
);

export default Pristatymas;